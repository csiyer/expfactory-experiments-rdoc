...
var createTrialTypes = function (numTrialsPerBlock, delay) {
  ...
  for (
    var numIterations = 0;
    numIterations < numTrialsPerBlock / n_back_conditions.length;
    numIterations++
  ) {
    for (
      var numNBackConds = 0;
...
var start_control_block = {
  ...
  stimulus:
    ...
    "<p class = block-text>Press your " +
    possible_responses[0][0] +
    " if the current letter was a T (or t) and your " +
    possible_responses[1][0] +
    " if not.</p> " +
  ...
  on_finish: function () {
    feedback_text =
      "We will now start this block. Press <i>enter</i> to begin.";
  },
};
...
var n_back_rdoc_init = () => {
  ...
  n_back_conditions = jsPsych.randomization.repeat(
